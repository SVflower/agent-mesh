    ...
    let runtimes = RUNTIME_DEFINITIONS
        ...
        .map(|definition| {
            let configured_candidates =
                configured_runtime_candidates(config.as_ref(), definition.kind);
...
fn list_runtime_adapters() -> Vec<RuntimeAdapterStatus> {
    vec![
        ...
        RuntimeAdapterStatus {
            ...
            session_status: "planned",
            notes:
                "OpenClaw 当前仅做手动 Persona 维护；自动发现验证前，不把 CoCo 等配置视为真实可用。",
        },
        RuntimeAdapterStatus {
            runtime_kind: "codex",
            display_name: "Codex",
    ...
    let mut rows = personas
        ...
        .map(|mut persona| {
            ...
            if let Some(object) = persona.as_object_mut() {
                object.insert(
                    "runtime".to_string(),
                    runtimes.get(&runtime_id).cloned().unwrap_or(Value::Null),
                );
            }
            persona
        })
    ...
    let mut rows = personas
        ...
        .filter_map(|persona| {
            ...
            let in_offices = office_members
                .values()
                .filter(|member| {
                    member.get("persona_id").and_then(Value::as_str) == Some(persona_id.as_str())
                })
                .filter_map(|member| {
                    member
                        .get("office_id")
                ...
                .collect::<Vec<_>>();
            let status =
                local_agent_status(&runtime_kind, &source, enabled, !in_offices.is_empty());
            let diagnostic = local_agent_diagnostic(
                &runtime_kind,
                &source,
                enabled,
                &name,
...
#[tauri::command]
fn delete_office(office_id: String) -> Result<Value, String> {
    let mut config = read_config()?;

    // Remove the office itself
    if let Some(offices) = config.get_mut("offices").and_then(Value::as_object_mut) {
        offices.remove(&office_id);
    }

    // Remove all members belonging to this office
    if let Some(members) = config
        .get_mut("officeMembers")
        .and_then(Value::as_object_mut)
    {
        let to_remove: Vec<String> = members
            .iter()
            .filter(|(_, member)| {
                member.get("office_id").and_then(Value::as_str) == Some(&office_id)
            })
            .map(|(id, _)| id.clone())
            .collect();
        for id in to_remove {
    ...
    // Remove routing policies belonging to this office
    if let Some(policies) = config
        .get_mut("routingPolicies")
        .and_then(Value::as_object_mut)
    {
        let to_remove: Vec<String> = policies
            .iter()
            .filter(|(_, policy)| {
                policy.get("office_id").and_then(Value::as_str) == Some(&office_id)
            })
            .map(|(id, _)| id.clone())
            .collect();
        for id in to_remove {
    ...
    // Remove task types that are scoped to this office (id contains office_id)
    if let Some(task_types) = config.get_mut("taskTypes").and_then(Value::as_object_mut) {
        let to_remove: Vec<String> = task_types
            .keys()
            .filter(|id| id.contains(&office_id))
            .cloned()
            .collect();
        for id in to_remove {
            task_types.remove(&id);
        }
    }
...
fn cancel_task(task_id: String) -> Result<Value, String> {
    ...
    push_task_event(
        ...
        "cancelled",
        if killed {
            "Task cancelled and worker process tree was terminated."
        } else {
            "Task cancelled; worker process tree termination was not confirmed."
        },
        Some(now.clone()),
    );
...
fn reset_task_session(task_id: String) -> Result<Value, String> {
    ...
    if let Some(object) = session.as_object_mut() {
        ...
        object.insert(
            "removed_session_file".to_string(),
            Value::Bool(removed_session_file),
        );
    }
    ...
    push_task_event(
        &mut task,
        "session_reset",
        "Task session marked as reset from Agent Mesh desktop.",
        Some(now.clone()),
    );
    write_task_file(&task_path, &task)?;
    append_jsonl_event(
...
fn create_chat_task(payload: CreateChatTaskPayload) -> Result<Value, String> {
    ...
    let workspace_path = resolve_task_workspace(&config, &payload.office_id, &root);
    let team_process = build_team_process_snapshot(&config, &payload, &workspace_path);
    let routing_decision = team_process
        .get("routingDecision")
        .cloned()
        .unwrap_or(Value::Null);
    let delegation = team_process
        .get("delegation")
        .cloned()
        .unwrap_or(Value::Null);
    let permission_decision = team_process
    ...
    let task = serde_json::json!({
      ...
      "routingDecision": routing_decision.clone(),
      "delegation": delegation.clone(),
      "permissionDecision": permission_decision.clone(),
      "task": {
        "id": task_id,
        "type": "task.assign",
        ...
        "workspace": {
          "repo": workspace_path.display().to_string()
        },
        "context": {
      ...
      "events": [
        ...
        },
        {
          ...
          "message": "Agent Mesh selected an OfficeMember by routing policy.",
          "data": routing_decision.clone()
        },
        {
          "at": now,
          "status": "delegation_created",
          "message": "Delegation record created for team observability.",
          "data": delegation.clone()
        },
        {
          "at": now,
          "status": "permission_decision",
          "message": "Permission policy resolved for this task.",
          "data": permission_decision.clone()
        }
      ],
      "result": null
    });
    ...
    let mut events =
        fs::File::create(task_dir.join("events.jsonl")).map_err(|error| error.to_string())?;
    writeln!(
        events,
        "{}",
        serde_json::json!({
...
fn resolve_task_workspace(config: &Value, office_id: &str, root: &Path) -> PathBuf {
    let configured = config
        .get("offices")
        .and_then(Value::as_object)
        .and_then(|offices| offices.get(office_id))
        .and_then(|office| office.get("default_workspace_path"))
        .and_then(Value::as_str)
...
fn build_team_process_snapshot(
    config: &Value,
    payload: &CreateChatTaskPayload,
    workspace_path: &Path,
) -> Value {
    let office = config
        .get("offices")
        .and_then(Value::as_object)
        .and_then(|offices| offices.get(&payload.office_id));
    let routing_policy = config
        .get("routingPolicies")
        .and_then(Value::as_object)
        .and_then(|policies| policies.get(&payload.routing_policy_id));
    let task_type = config
        .get("taskTypes")
        .and_then(Value::as_object)
        .and_then(|task_types| task_types.get(&payload.task_type_id));
    let member = config
        .get("officeMembers")
        .and_then(Value::as_object)
        .and_then(|members| members.get(&payload.assigned_member_id));
    ...
    let permission_policy_id = office
        .and_then(|value| value.get("default_permission_policy_id"))
        .and_then(Value::as_str)
        .unwrap_or_default();
    let permission_policy = config
        .get("permissionPolicies")
        .and_then(Value::as_object)
        .and_then(|policies| policies.get(permission_policy_id));
    let permission_mode = permission_policy
        .and_then(|value| value.get("default_mode"))
        .and_then(Value::as_str)
        .unwrap_or("safe-write");
    let strategy = routing_policy
        .and_then(|value| value.get("strategy"))
        .and_then(Value::as_str)
        .unwrap_or("capability-match");
    let required_capabilities = routing_policy
        .and_then(|value| value.get("required_capabilities"))
        .cloned()
        .unwrap_or_else(|| serde_json::json!([]));
    let task_required_capabilities = task_type
        .and_then(|value| value.get("required_capabilities"))
        .cloned()
        .unwrap_or_else(|| serde_json::json!([]));
    let reason = format!(
        "Routing policy '{}' used strategy '{}' and selected member '{}' for task type '{}'.",
        routing_policy
            .and_then(|value| value.get("name"))
            .and_then(Value::as_str)
            .unwrap_or(&payload.routing_policy_id),
        strategy,
        member
            .and_then(|value| value.get("office_title"))
            .and_then(Value::as_str)
            .unwrap_or(&payload.assigned_member_id),
        task_type
            .and_then(|value| value.get("name"))
            .and_then(Value::as_str)
            .unwrap_or(&payload.task_type_id)
    );

    serde_json::json!({
      "routingDecision": {
        "routing_policy_id": payload.routing_policy_id,
        "routing_policy_name": routing_policy.and_then(|value| value.get("name")).and_then(Value::as_str),
        "strategy": strategy,
        "reason": reason.clone(),
        "required_capabilities": required_capabilities,
        "task_required_capabilities": task_required_capabilities,
        "selected_member_id": payload.assigned_member_id,
        "selected_member_title": member.and_then(|value| value.get("office_title")).and_then(Value::as_str),
        "selected_persona_id": persona_id,
        "selected_persona_name": persona.and_then(|value| value.get("name")).and_then(Value::as_str),
        "selected_runtime_id": runtime_id,
        "selected_runtime_kind": runtime.and_then(|value| value.get("kind")).and_then(Value::as_str)
      },
      "delegation": {
        "id": format!("delegation_{}", payload.assigned_member_id),
        "requester": "agent-mesh-chat",
        "office_id": payload.office_id,
        "office_name": office.and_then(|value| value.get("name")).and_then(Value::as_str),
        "assignee_member_id": payload.assigned_member_id,
        "assignee_title": member.and_then(|value| value.get("office_title")).and_then(Value::as_str),
        "assignee_responsibility": member.and_then(|value| value.get("responsibility")).and_then(Value::as_str),
        "assignee_persona_id": persona_id,
        "assignee_runtime_kind": runtime.and_then(|value| value.get("kind")).and_then(Value::as_str),
        "reason": reason.clone(),
        "workspace_path": workspace_path.display().to_string()
      },
      "permissionDecision": {
        "policy_id": permission_policy_id,
        "policy_name": permission_policy.and_then(|value| value.get("name")).and_then(Value::as_str),
        "mode": permission_mode,
        "decision": if permission_mode == "read-only" { "allow_read_only" } else { "allow_with_policy" },
        "write_detection_required": permission_mode == "read-only",
        "require_confirmation": permission_policy.and_then(|value| value.get("require_confirmation")).cloned().unwrap_or_else(|| serde_json::json!([]))
      }
    })
}
...
fn dispatch_chat_task(task_id: String) -> Result<Value, String> {
    ...
    let resolved_command = resolve_runtime_command(&candidates)?.ok_or_else(|| {
        format!(
    ...
    append_jsonl_event(
        ...
        serde_json::json!({
          ...
          "source": "agent-mesh-dispatcher",
          "office_id": task.get("office_id").and_then(Value::as_str),
          "assigned_member_id": task.get("assigned_member_id").and_then(Value::as_str),
          "runtime": adapter.runtime_kind
        }),
    )?;
    push_task_event(
        &mut task,
        "assigned",
        &format!("Assigned to {} runtime.", adapter.display_name),
        Some(now.clone()),
    );
    ...
    if let Some(object) = task.as_object_mut() {
        object.insert(
            "agent".to_string(),
            Value::String(adapter.runtime_kind.to_string()),
        );
        object.insert(
            "agentType".to_string(),
            Value::String(adapter.agent_type.to_string()),
        );
    }
    write_task_file(&task_path, &task)?;
...
fn detect_runtime(
    ...
    let executable_path = resolved_command
        .as_ref()
    ...
    RuntimeDetection {
        ...
        diagnostic: if let Some(error) = resolution_error {
            format!(
                "Runtime command resolution failed: {error}. Tried: {}.",
                format_candidate_list(&candidates)
            )
...
fn run_cli_chat_worker(
    ...
    let execution_root = task_execution_root(&task, &root);
    let skip_permissions_removed = adapter
        .invocation_args
        .iter()
    ...
    let _ = append_jsonl_event(
        ...
        serde_json::json!({
          ...
          "cwd": execution_root.display().to_string()
        }),
    );
    let _ = append_jsonl_event(
        &task_dir,
        serde_json::json!({
          "at": started_at,
          "taskId": task_id,
          "type": "permission_enforced",
          "source": "agent-mesh-permission",
          "permission_mode": permission_mode,
          "skip_permissions_removed": skip_permissions_removed,
          "write_detection": permission_mode == "read-only"
        }),
    );
    let _ = push_and_write_task_event(
        &task_path,
        &mut task,
        "running",
        &format!("{} worker started.", adapter.display_name),
        Some(started_at),
    );

    let git_before = if permission_mode == "read-only" {
        let snapshot = git_status_snapshot(&execution_root);
        if snapshot.is_none() {
            let _ = append_jsonl_event(
                &task_dir,
                serde_json::json!({
                  "at": iso_timestamp(),
                  "taskId": task_id,
                  "type": "permission_check_unavailable",
                  "source": "agent-mesh-permission",
                  "permission_mode": "read-only",
                  "reason": "git status snapshot before execution failed",
                  "cwd": execution_root.display().to_string()
                }),
            );
        }
    ...
    let output = run_agent_process(
        ...
        |pid| {
            ...
            record_worker_pid(&task_path, &task_dir, &task_id, pid, at)
                .map_err(|error| format!("Failed to record worker pid {pid}: {error}"))
        },
    );
    ...
    match output {
        Ok(process_output) => {
            ...
            if let Some(before) = &git_before {
                if let Some(after) = git_status_snapshot(&execution_root) {
                    ...
                    if !violations.is_empty() {
                        if let Some(obj) = result.as_object_mut() {
                            ...
                            obj.insert(
                                "write_detection_status".to_string(),
                                Value::String("violated".to_string()),
                            );
                            obj.insert(
                                "write_violations".to_string(),
                                serde_json::json!(violations),
                            );
                        }
                        let _ = append_jsonl_event(
                    ...
                    if let Some(obj) = result.as_object_mut() {
                        ...
                        obj.insert(
                            "write_detection_error".to_string(),
                            Value::String("git status snapshot after execution failed".to_string()),
                        );
                    }
                    let _ = append_jsonl_event(
                        &task_dir,
                        serde_json::json!({
                          ...
                          "source": "agent-mesh-permission",
                          "permission_mode": "read-only",
                          "reason": "git status snapshot after execution failed",
                          "cwd": execution_root.display().to_string()
                        }),
                    );
                }
            } else if permission_mode == "read-only" {
                if let Some(obj) = result.as_object_mut() {
                    obj.insert(
                        "write_detection_status".to_string(),
                        Value::String("unavailable".to_string()),
                    ...
                    obj.insert(
                        "write_detection_error".to_string(),
                        Value::String("git status snapshot before execution failed".to_string()),
                    );
                }
            }

            let persisted_session =
                persist_runtime_session_from_output(&process_output, &execution_root, &finished_at)
                    .ok()
                    .flatten();
            let status = result
                .get("status")
            ...
            let persisted_task_session = persisted_session.as_ref().map(|session| {
                task_session_from_runtime_session(&task, &task_id, session, final_status)
            });

            if let Some(object) = task.as_object_mut() {
                object.insert(
                    "status".to_string(),
                    Value::String(final_status.to_string()),
                );
                object.insert("updatedAt".to_string(), Value::String(finished_at.clone()));
                object.insert(
                    "completedAt".to_string(),
                    Value::String(finished_at.clone()),
                );
        ...
        Err(error) => {
            ...
            push_task_event(
                &mut task,
                "failed",
                &format!("{} worker failed before completion.", adapter.display_name),
                Some(finished_at.clone()),
            );
            let _ = write_task_file(&task_path, &task);
            let _ = append_jsonl_event(
    ...
    F: FnOnce(u32) -> Result<(), String>,
{
    ...
    let mut child = command
        .spawn()
        .map_err(|error| format!("Failed to spawn {}: {error}", executable_path.display()))?;
    let pid = child.id();
    on_started(pid)?;
    if let Some(mut stdin) = child.stdin.take() {
        stdin
            .write_all(stdin_text.as_bytes())
            .map_err(|error| format!("Failed to write prompt to stdin: {error}"))?;
    }
    let output = child
        .wait_with_output()
...
fn build_office_context_section(config: &Value, task: &Value) -> String {
    ...
    let office = config
        .get("offices")
        .and_then(Value::as_object)
        .and_then(|o| o.get(office_id));
    let office_name = office
        .and_then(|o| o.get("name"))
        .and_then(Value::as_str)
        .unwrap_or(office_id);
    let office_desc = office
        .and_then(|o| o.get("description"))
        .and_then(Value::as_str)
        .unwrap_or("");

    let assigned_member_id = task_body
        .get("assigned_member_id")
        .and_then(Value::as_str)
        .unwrap_or_default();
    let member = config
    ...
    let member_title = member
        .and_then(|m| m.get("office_title"))
        .and_then(Value::as_str)
        .unwrap_or("执行成员");
    let member_responsibility = member
        .and_then(|m| m.get("responsibility"))
        .and_then(Value::as_str)
        .unwrap_or("");
    ...
    if permission_mode == "read-only" {
        section.push_str(
            "⚠️ 严禁任何写操作：不得创建、修改、删除文件，不执行改变工作区状态的命令。\n",
        );
    }
...
fn record_worker_pid(
    ...
    push_task_event(
        &mut task,
        "running",
        &format!("Agent worker started with pid {pid}."),
        Some(at.clone()),
    );
    write_task_file(task_path, &task)?;
    append_jsonl_event(
...
fn update_task_status(task: &mut Value, status: &str, at: Option<String>) {
    if let Some(object) = task.as_object_mut() {
        ...
        object.insert(
            "updatedAt".to_string(),
            Value::String(at.unwrap_or_else(iso_timestamp)),
        );
    }
}
...
fn write_task_file(task_path: &Path, task: &Value) -> Result<(), String> {
    ...
    let tmp_path = task_path.with_extension(format!("{}.tmp", current_timestamp_millis()));
    fs::write(&tmp_path, content).map_err(|error| error.to_string())?;
    if task_path.exists() {
        fs::remove_file(task_path).map_err(|error| error.to_string())?;
    }
    fs::rename(&tmp_path, task_path).map_err(|error| error.to_string())
}
...
fn task_session_from_runtime_session(
    task: &Value,
    task_id: &str,
    session: &Value,
    task_status: &str,
...
fn persist_runtime_session_from_output(
...
fn runtime_session_path(repo: &Path) -> Result<PathBuf, String> {
    Ok(workspace_root()?
        .join(".agent-mesh")
        .join("sessions")
        .join(format!("{}.json", repo_session_key(repo))))
}
...
fn task_execution_root(task: &Value, app_root: &Path) -> PathBuf {
    task_workspace_repo(task)
        .map(PathBuf::from)
        .map(|path| {
            if path.is_absolute() {
                path
...
fn resolve_single_command(candidate: &str, root: &Path) -> Option<PathBuf> {
    ...
    // On Windows, if candidate has no extension, try .cmd/.exe/.ps1 first to avoid
    // matching invalid extensionless files (e.g. Unix shell stubs installed by npm).
    let has_extension = candidate_path.extension().is_some();
    if cfg!(windows) && !has_extension {
        let win_suffixes = [".cmd", ".exe", ".ps1"];
        for suffix in win_suffixes {
            let suffixed = format!("{candidate}{suffix}");
            let found = env::split_paths(&path_value)
                .map(|directory| directory.join(&suffixed))
...
mod tests {
    ...
    fn reports_adapter_status_for_four_mvp_runtimes() {
        ...
        let kinds = statuses
            .iter()
            .map(|status| status.runtime_kind)
            .collect::<Vec<_>>();
    ...
    fn missing_command_returns_none() {
        ...
        assert!(resolve_runtime_command(&candidates)
    ...
    fn resolves_configured_relative_path_from_workspace_root() {
        ...
        let file_name = if cfg!(windows) {
            "agent-mesh-test-runtime.cmd"
        } else {
            "agent-mesh-test-runtime"
        };
    ...
    fn replace_task_id_updates_nested_task_and_observer() {
        ...
        replace_task_id(
            &mut task,
            "new",
            Path::new("D:\\IDEA\\workspace\\agent-mesh\\.agent-mesh"),
        );
    ...
    fn extracts_session_id_from_claude_stdout_raw() {
        ...
        assert_eq!(
            extract_runtime_session_id(&task),
    ...
    fn builds_task_session_status_from_task() {
        ...
        assert_eq!(
            session.get("runtime_session_id").and_then(Value::as_str),
            Some("session-123")
        );
        assert_eq!(
            session.get("office_member_id").and_then(Value::as_str),
            Some("member_claude")
        );
    }
    ...
    #[test]
    fn team_process_snapshot_records_routing_delegation_and_permission() {
        let config = serde_json::json!({
          "offices": {
            "office_test": {
              "id": "office_test",
          ...
          "officeMembers": {
            "member_1": {
              ...
              "office_id": "office_test",
              "persona_id": "persona_claude",
              "office_title": "只读检查 Agent",
              "responsibility": "只读检查仓库"
            }
          },
          "personas": {
            "persona_claude": {
        ...
        let payload = CreateChatTaskPayload {
            office_id: "office_test".to_string(),
            task_type_id: "task_readonly".to_string(),
            routing_policy_id: "routing_readonly".to_string(),
            assigned_member_id: "member_1".to_string(),
            objective: "检查项目".to_string(),
            context: String::new(),
            acceptance: vec![],
            constraints: vec![],
        };

        let snapshot = build_team_process_snapshot(
            &config,
            &payload,
            Path::new("D:\\IDEA\\workspace\\agent-mesh"),
        );

        assert_eq!(
            snapshot
                .get("routingDecision")
                .and_then(|value| value.get("selected_runtime_kind"))
                .and_then(Value::as_str),
            Some("claude-code")
        );
        assert_eq!(
            snapshot
                .get("delegation")
                .and_then(|value| value.get("assignee_title"))
                .and_then(Value::as_str),
            Some("只读检查 Agent")
        );
        assert_eq!(
    ...
    #[test]
    fn resolves_office_workspace_from_default_workspace_path() {
        let config = serde_json::json!({
          "offices": {
            "office_test": {
              "id": "office_test",
    ...
    #[test]
    fn task_execution_root_prefers_task_workspace_over_app_root() {
        let app_root = Path::new("D:\\IDEA\\workspace\\agent-mesh");
        let task = serde_json::json!({
          "task": {
            "workspace": {
              "repo": "D:\\IDEA\\workspace\\other-repo"
            }
          }
        });

        assert_eq!(
            task_execution_root(&task, app_root),
            PathBuf::from("D:\\IDEA\\workspace\\other-repo")
        );
    }

    #[test]
    fn task_execution_root_resolves_relative_workspace_from_app_root() {
        let app_root = Path::new("D:\\IDEA\\workspace\\agent-mesh");
        let task = serde_json::json!({
          "task": {
            "workspace": {
              "repo": "examples/read-only-target"
            }
          }
        });
        ...
        assert_eq!(
            task_execution_root(&task, app_root),
            PathBuf::from("D:\\IDEA\\workspace\\agent-mesh").join("examples/read-only-target")
        );
    }

    #[test]
    #[ignore = "Runs a real Claude Code read-only task through Agent Mesh Chat dispatch."]
    fn real_readonly_chat_dispatch_smoke() {
        let config = read_config().expect("agent-mesh config should load");
        let (office_id, task_type_id, routing_policy_id, member_id) =
            find_readonly_claude_smoke_target(&config)
                .expect("read-only Claude Code office should exist");
        let created = create_chat_task(CreateChatTaskPayload {
      office_id,
      task_type_id,
      routing_policy_id,
      assigned_member_id: member_id,
      objective: "只读检查 agent-mesh 项目是做什么的，说明核心模块、当前能力边界和下一步最该验证的闭环。不得修改任何文件。".to_string(),
      context: "这是阶段 6.5 真实闭环 smoke test，必须只读检查当前仓库。".to_string(),
      acceptance: vec![
        "说明 agent-mesh 项目是做什么的".to_string(),
        "列出核心模块和当前已具备能力".to_string(),
        "确认没有写入、删除、移动或格式化任何文件".to_string(),
      ],
      constraints: vec![
        "只读检查，不修改文件".to_string(),
        "不要执行会改变工作区状态的命令".to_string(),
      ],
    }).expect("chat task should be created");
        let task_id = created
            .get("id")
            .and_then(Value::as_str)
        ...
        dispatch_chat_task(task_id.clone()).expect("chat task should dispatch");
        let task_path = workspace_root()
            .expect("workspace root should resolve")
            .join(".agent-mesh")
            .join("tasks")
            .join(format!("{task_id}.json"));
        let mut final_task = read_task_file_with_retry(&task_path).expect("task file should exist");
        for _ in 0..90 {
            let status = final_task
                .get("status")
                .and_then(Value::as_str)
                .unwrap_or("unknown");
            if matches!(status, "completed" | "failed" | "cancelled") {
            ...
            final_task =
                read_task_file_with_retry(&task_path).expect("task file should remain readable");
        }
        ...
        assert!(
            ...
            "task should reach terminal status, got {status}"
        );
        assert_eq!(
            final_task
                .get("permissionDecision")
        ...
        assert!(
            final_task.get("routingDecision").is_some(),
            "routing decision should be recorded"
        );
        assert!(
            final_task.get("delegation").is_some(),
            "delegation should be recorded"
        );
    }

    fn find_readonly_claude_smoke_target(
        config: &Value,
    ) -> Option<(String, String, String, String)> {
        let offices = config.get("offices")?.as_object()?;
        let policies = config.get("permissionPolicies")?.as_object()?;
        let members = config.get("officeMembers")?.as_object()?;
        let personas = config.get("personas")?.as_object()?;
        let runtimes = config.get("runtimes")?.as_object()?;

        for office in offices.values() {
            let office_id = office.get("id")?.as_str()?;
            let policy_id = office.get("default_permission_policy_id")?.as_str()?;
            if policies.get(policy_id)?.get("default_mode")?.as_str()? != "read-only" {
                continue;
            }
            let task_type_id = office.get("default_task_type_id")?.as_str()?.to_string();
            let routing_policy_id = office
                .get("default_routing_policy_id")?
                .as_str()?
                .to_string();
            let member = members.values().find(|member| {
                if member.get("office_id").and_then(Value::as_str) != Some(office_id) {
                    return false;
                }
                let persona_id = match member.get("persona_id").and_then(Value::as_str) {
                    Some(value) => value,
                    None => return false,
                };
                let runtime_id = match personas
                    .get(persona_id)
                    .and_then(|persona| persona.get("runtime_id"))
                    .and_then(Value::as_str)
                {
                    Some(value) => value,
                    None => return false,
            ...
            return Some((
                ...
                task_type_id,
                routing_policy_id,
                member.get("id")?.as_str()?.to_string(),
            ));
        }
    ...
    fn read_task_file_with_retry(path: &Path) -> Result<Value, String> {
        ...
        Err(last_error.unwrap_or_else(|| "task file was not readable".to_string()))
    }
}
